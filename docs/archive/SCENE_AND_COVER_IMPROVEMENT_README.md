...
- **24 Ocak 2026:** v1.5.0 uygulandı (Analiz tabanlı plan Faz 1–5): **Moderation 400 → 1 retry** (cover edits); **yaş kısıtları kaldırıldı** (getAgeAppropriateSceneRules age-agnostic); **Character centered kaldırıldı**, rule of thirds / leading lines eklendi (first interior); **Kapak prompt yumuşatma** (integrated into environment, integrated into scene). ROADMAP 3.5.24 eklendi (tüm kitap fail – sadece madde, impl. sonra). Dokümantasyon güncellendi.
- **24 Ocak 2026:** 2 sayfa test (Uzay Macerası) geri bildirimi: **Kapak** "sanatsal havası yok, çok güzel değil" → neden: kapak hâlâ "character centered, clear face" (focusPoint: character); **Sayfa 1** "harika, tam istediğim gibi" → balanced + NOT centered + wide + rule of thirds + rich env; **Sayfa 2** "karakter çok öne çıkarılmış" → **close-up** perspektifi. Analiz ve öneriler [KAPAK_BOS_KARAKTER_ODAKLI_ANALIZ.md §5](./KAPAK_BOS_KARAKTER_ODAKLI_ANALIZ.md) (kapak: character centered kaldır; sayfa 2+: close-up kaldır).
- **24 Ocak 2026:** **Plan: Kapak/Close-up/Kıyafet** uygulandı (v1.6.0 / Story v1.3.0): Kapak `focusPoint` → balanced; close-up kaldırıldı (getCameraAngleDirectives, getPerspectiveForPage); **story-driven clothing** (StoryPage.clothing, SceneInput.clothing, story prompt + JSON, books route, generateFullPagePrompt). Dokümantasyon güncellendi (CHANGELOG, KAPAK_BOS §5.5).
