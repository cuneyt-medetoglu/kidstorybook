    ...
    try {
      ...
      if (result?.error) {
        ...
        if (msg.includes("email not confirmed") || msg.includes("Email not confirmed")) {
          throw new Error(t("login.errors.verifyEmail"))
        }
      ...
      const errorMessage = err instanceof Error ? err.message : t("login.errors.generic")
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="rememberMe" className="text-sm font-medium text-gray-700 dark:text-slate-300 cursor-pointer">
                    {t("login.rememberMe")}
                  </Label>
                </div>
                <Link href="/auth/forgot-password" className="text-sm font-medium text-primary underline underline-offset-2 transition-colors hover:text-brand-2">
                  {t("login.forgotPassword")}
                </Link>
              </motion.div>

              {error && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg bg-red-50 p-3 dark:bg-red-900/20">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9, duration: 0.4 }} className="relative">
            ...
            >
              <p className="text-sm text-gray-600 dark:text-slate-400">
                {t("login.noAccount")}{" "}
                <Link href="/auth/register" className="font-semibold text-primary underline underline-offset-2 transition-colors hover:text-brand-2">
                  {t("login.signUpLink")}
                </Link>
              </p>
            </motion.div>
