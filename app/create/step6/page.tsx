          ...
          <div className="rounded-2xl bg-white/80 p-6 shadow-2xl backdrop-blur-sm dark:bg-slate-800/80 md:p-8">
            ...
            >
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.8, duration: 0.4 }}
                className="mb-4 rounded-lg border-2 border-yellow-500 bg-yellow-50 p-4 dark:border-yellow-400 dark:bg-yellow-900/20"
              >
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // Şimdilik mock characterId kullanıyoruz veya localStorage'dan alıyoruz
                        ...
                        console.log("[Step 6] Create book response:", result)
                        console.log("[Step 6] Response status:", response.status)
                        console.log("[Step 6] Response ok:", response.ok)

                        if (!response.ok || !result.success) {
                          console.error("[Step 6] Create book failed:", result)
                          throw new Error(result.message || result.error || "Failed to create book")
                        }

                        // Success! API returns: { success: true, data: { id, title, status, ... }, message: "..." }
                        const bookId = result.data?.id
                        const bookTitle = result.data?.title || "Your book"

                        console.log("[Step 6] Book created successfully:", { bookId, bookTitle, fullResponse: result })

                        toast({
                          title: "Book Created!",
                          description: `"${bookTitle}" is being generated. Redirecting...`,
                        })

                        // Clear wizard data
                        localStorage.removeItem("kidstorybook_wizard")
                        localStorage.removeItem("kidstorybook_character_id")

                        // Redirect to dashboard first (books/:id/view later)
                        // TODO: After story generation is tested, redirect to book view
                        setTimeout(() => {
                          if (bookId) {
                            console.log("[Step 6] Redirecting to dashboard, book ID:", bookId)
                            // For now, redirect to dashboard (book view will be tested separately)
                            router.push("/dashboard")
                            // TODO: After Faz 1 test passes, redirect to: router.push(`/books/${bookId}/view`)
                          } else {
                            console.warn("[Step 6] No book ID in response, redirecting to dashboard")
                            router.push("/dashboard")
                          }
                        }, 2000)
                      } catch (error) {
                    ...
                    className="w-full bg-gradient-to-r from-purple-500 to-pink-500 px-8 py-8 text-lg font-bold text-white shadow-lg transition-all hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed dark:from-purple-400 dark:to-pink-400 sm:w-auto"
                  >
                    {isCreating ? (
                      <>
